...
class Permission extends OctoUsersAppModel {
	...
	public function userPermissions($user) {
		$roles = Set::extract('/Role/id', $user);
		
		$permissions = $this->find('all', array(
			'conditions' => array('Permission.role_id' => $roles),
			'contain' => 'Controller'
		));
		...
		foreach($permissions as $permission) {
			...
			foreach($permission['Permission'] as $crud => $value) {
				if(strpos($crud, '_') === 0) {
					$action = ltrim($crud, '_');
					if(isset($return[$controller][$action])) {
						$return[$controller][$action] = $return[$controller][$action] || $value;
					}
					else {
