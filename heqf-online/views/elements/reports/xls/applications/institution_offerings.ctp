<?php
	...
	$title = 'Report - Offerings per institution : ' . $Institution[$this->params['named']['institution_id']] . "\n" . date('Y-m-d H:i');
	$fileName = 'Report - Offerings per institution_' . $Institution[$this->params['named']['institution_id']];
		...
		foreach($reportData as $qualType => $applications){
			...
			foreach($applications as $application){
				...
				*/
				...
				$worksheet->getColumnDimensionByColumn($column)->setAutoSize(true);
				$worksheet->setCellValueByColumnAndRow($column++, $row, date('d F Y', strtotime($application['outcome_approval_date'])));
				$worksheet->getColumnDimensionByColumn($column)->setAutoSize(true);
				$row++;
			}
		}
