			...
			function generateDialog(result) {
				...
				if($.isPlainObject(result)) {
					...
					if($(".ui-dialog").is(":visible")) {
						//add datepicket to teachout date if it exists
						if ($(".ui-dialog").find('#HeqfQualificationS1TeachoutDate').length) {
							$(".ui-dialog").find('#HeqfQualificationS1TeachoutDate').datepicker({ 
								changeMonth: true,
								changeYear: true,
								dateFormat: 'yy-mm-dd'
							});
